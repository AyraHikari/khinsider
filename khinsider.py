	...
	def download(self, path='', makeDirs=True, formatOrder=None, verbose=False):
		...
		with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
			for song in self.songs:
				songs_thread.append(executor.submit(getAppropriateFile, song, formatOrder))
				print("Got {} song(s)...".format(len(songs_thread)))

		files = [future.result() for future in songs_thread]
		print("Got all {} songs...".format(len(files)))
		...
		for fileNumber, file in enumerate(files, 1):
			thread = threading.Thread(target=friendlyDownloadFile, args=(file, path, fileNumber, totalFiles, verbose,))
		...
		for thread in threads:
			results.append(thread.is_alive())
